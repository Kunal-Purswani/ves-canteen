            ...
            for (users in data) {
                for (order in data[users]) {
                    if (!data[users][order].Delivery_Status) {
                        ...
                        text += '<td><a target="_blank" rel="noopener noreferrer" class="ord-details btn btn-primary" href="https://kunal-purswani.github.io/order-details.html?id=' + order + '">Order Detail</a></button></td>'
                        text += '</tr>\n'
                    }
                }
            }
